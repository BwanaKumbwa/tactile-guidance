...
class BeltAdapter(FeedbackDevice):
    ...
    def __init__(self, virtual_belt_controller):
        ...
        # While > now, navigation must not overwrite calibration vibrations
        ...
        # Motor index currently being inspected during step-through calibration
        self._probe_index = 0
        # Which bit-index sits on the green marker (navel). None → use ANGLE 0°.
        self._navel_motor_index: Optional[int] = None
        # +1: index increases toward the right; -1: toward the left
        self._motor_dir = 1
    ...
    def update(self, ctx: NavigationContext) -> Optional[object]:
        ...
        frame_w, _frame_h = self._frame_size(ctx)
        tx = float(target[0])
        ...
        if now < self._cue_hold_until:
            # Let the cue pulse play without continuous steering overwriting it
            return target
    ...
    def _enter_handoff(self) -> None:
        ...
        if self._currently_vibrating:
            self.stop()
        # Avoid pre-handoff cascade if the user later backs out of handoff
    ...
    def _bearing_deg(self, target_x: float, frame_w: int) -> float:
        """
        Map horizontal target offset to a body angle around the waist.

        0°   = navel / green marker (straight ahead)
        90°  = right
        270° = left

        Phone is assumed facing the same way as the user.
        """
        cx = frame_w * 0.5
    ...
    def _vibrate(self, angle_deg: float, intensity: int) -> None:
        ...
        if self._navel_motor_index is not None:
            # Calibrated: drive the physical motor under the body direction
            ...
            # Uncalibrated fallback: ANGLE mode (0° = belt-local front)
            self._virtual_belt.send_vibration_command(
                channel_index=1,
                pattern=0,
                intensity=inten,
