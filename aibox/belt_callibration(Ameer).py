...
Usage:
    from belt import connect_navigation_belt, NavigationBeltController, load_belt_calibration

    ok, hw = connect_navigation_belt()
    calibration = load_belt_calibration(participant=1)
    nav = NavigationBeltController(vibration_intensities=calibration)
    switch, target = nav.navigate_body(hw, bboxes, target_cls, hand_clss, depth_img)
"""
...
root = file.parents[1] if file.parent.name == 'resources' else file.parent
...
DIRECTION_LABELS = {
    'N':  'Front (North)',
    'NE': 'Front-Right (North-East)',
    'E':  'Right (East)',
    'SE': 'Back-Right (South-East)',
    'S':  'Back (South)',
    'SW': 'Back-Left (South-West)',
    'W':  'Left (West)',
    'NW': 'Front-Left (North-West)',
}

DEFAULT_INTENSITY = 50
DEFAULT_SWITCH_DISTANCE = 1.5  # metres
...
def load_belt_calibration(participant, calibration_dir=None):
    """
    Load per-direction vibration intensities from a calibration JSON file
    produced by belt_calibration.py.

    Args:
        participant (int): Participant ID number.
        calibration_dir (str or Path, optional): Directory containing calibration files.
            Defaults to <project_root>/results/calibration/

    Returns:
        dict: Mapping of direction → intensity, e.g. {'N': 50, 'NE': 45, ...}.
              Falls back to DEFAULT_INTENSITY for all directions if file not found.
    """
    ...
    calibration_file = calibration_dir / f'belt_calibration_participant_{participant}.json'
    ...
    if calibration_file.exists():
        try:
            ...
            # Validate that all 8 directions are present
            missing = [d for d in DIRECTIONS if d not in calibration]
            if missing:
                print(f"[NavBelt] Warning: calibration missing directions {missing}. "
                      f"Using default ({DEFAULT_INTENSITY}) for those.")
                for d in missing:
                    calibration[d] = DEFAULT_INTENSITY

            print(f"[NavBelt] Calibration loaded for participant {participant}:")
            for d in DIRECTIONS:
                print(f"  {DIRECTION_LABELS[d]:>30s}: {calibration[d]}")

            return calibration

        except (json.JSONDecodeError, KeyError) as e:
            print(f"[NavBelt] Error reading calibration file: {e}")

    # Fallback: default intensity for all directions
    print(f"[NavBelt] No calibration found for participant {participant}. "
          f"Using default intensity ({DEFAULT_INTENSITY}) for all directions.")
    return {d: DEFAULT_INTENSITY for d in DIRECTIONS}


def convert_bracelet_calibration(bracelet_calibration):
    """
    Convert a 4-direction bracelet calibration dict to 8-direction belt format.
    Useful when only bracelet calibration is available.

    Mapping:
        top    → N       bottom → S
        right  → E       left   → W
        Diagonals get the average of their two neighboring cardinals.

    Args:
        bracelet_calibration (dict): {'top': int, 'bottom': int, 'left': int, 'right': int}

    Returns:
        dict: 8-direction belt calibration.
    """
    top = bracelet_calibration.get('top', DEFAULT_INTENSITY)
    bottom = bracelet_calibration.get('bottom', DEFAULT_INTENSITY)
    left = bracelet_calibration.get('left', DEFAULT_INTENSITY)
    right = bracelet_calibration.get('right', DEFAULT_INTENSITY)

    return {
        'N':  top,
        'NE': int((top + right) / 2),
        'E':  right,
        'SE': int((bottom + right) / 2),
        'S':  bottom,
        'SW': int((bottom + left) / 2),
        'W':  left,
        'NW': int((top + left) / 2),
    }
...
class NavigationBeltController:
    ...
    def __init__(self, vibration_intensities=None, navigation_type=1,
                 switch_distance=DEFAULT_SWITCH_DISTANCE):
        """
        Args:
            vibration_intensities: dict with max vibration intensity per direction (0–100).
                                   Accepts either:
                                   - 8-direction belt format: {'N': 50, 'NE': 45, ...}
                                   - 4-direction bracelet format: {'top': 50, 'bottom': 50, ...}
                                   Defaults to DEFAULT_INTENSITY for all directions.
        ...
        if vibration_intensities is None:
            self.vibration_intensities = {d: DEFAULT_INTENSITY for d in DIRECTIONS}
        elif 'top' in vibration_intensities:
            # Bracelet-format calibration — convert to belt format
            self.vibration_intensities = convert_bracelet_calibration(vibration_intensities)
            print("[NavBelt] Converted bracelet calibration to 8-direction belt format.")
        else:
            # Already in belt format — validate
            self.vibration_intensities = {
                d: vibration_intensities.get(d, DEFAULT_INTENSITY)
                for d in DIRECTIONS
            }
        ...
        self.prev_intensities = {d: 0 for d in DIRECTIONS}
    ...
    # ----------------------------------------------------------------
    #  Calibration update (live adjustment)
    # ----------------------------------------------------------------

    def update_calibration(self, direction, intensity):
        """
        Update the vibration intensity for a single direction at runtime.
        Called when the user adjusts intensity via voice commands or MCP.

        Args:
            direction (str): One of 'N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'.
            intensity (int): New intensity value (5–100).
        """
        if direction in DIRECTIONS:
            intensity = max(5, min(100, intensity))
            self.vibration_intensities[direction] = intensity
            print(f"[NavBelt] {DIRECTION_LABELS[direction]} intensity → {intensity}")
        else:
            print(f"[NavBelt] Unknown direction: {direction}")

    def update_all_calibrations(self, calibration_dict):
        """
        Replace all vibration intensities at once.

        Args:
            calibration_dict (dict): Full 8-direction calibration.
        """
        for d in DIRECTIONS:
            if d in calibration_dict:
                self.vibration_intensities[d] = max(5, min(100, calibration_dict[d]))
        print("[NavBelt] All calibrations updated.")
    ...
    def choose_detection(self, bboxes, previous_bbox=None, w=640, h=640):
        ...
        for bbox in bboxes:
            if bbox[0] <= w and bbox[1] <= h:
                ...
                track_id_score = (track_id_weight
    ...
    # ----------------------------------------------------------------
    #  Direction & intensity mapping
    # ----------------------------------------------------------------

    def get_direction_intensities(self, target_x, target_y, frame_w, frame_h,
        ...
        Target to the right of center → East motors.
        Target above center → North motors.  Etc.
        ...
        intensities = {d: 0 for d in DIRECTIONS}

        if self.navigation_type == 0:
            # --- Single nearest direction ---
            nearest = self._angle_to_direction(angle)
            intensities[nearest] = int(
                vibration_intensities[nearest] * intensity_scale
            )
            ...
            lower_dir = DIRECTIONS[lower_idx]
            upper_dir = DIRECTIONS[upper_idx]
            ...
            if max(intensities.values()) < 5:
                intensities[lower_dir] = max(
                    5, int(vibration_intensities[lower_dir] * 0.3)
                )
    ...
    def navigate_body(self, belt_controller, bboxes, target_cls, hand_clss,
        ...
        Args:
            ...
            vibration_intensities: dict with max intensity per direction.
                                   If None, uses self.vibration_intensities.
        ...
        if target is not None:
            ...
            if current_time - self.last_vib_update_time > 0.2:
                ...
                if changed:
                    ...
                    if belt_controller and self.vibrate:
                        self._send_directional_vibration(
                            belt_controller, intensities
                        )

            if self.mock_navigate:
                active = {d: v for d, v in intensities.items() if v > 0}
                dist_str = (f"{depth_value:.2f}m"
                            if depth_value > 0 else "N/A")
                print(f"[NavBelt] Guiding — angle: {angle:.0f}° "
                      f"{active} dist: {dist_str}")
        ...
        # ==============================================================
            ...
            if belt_controller and self.vibrate:
                self._send_directional_vibration(
                    belt_controller, self.prev_intensities
                )
        ...
        for direction in DIRECTIONS:
            ...
            if intensity > 0 and channel < 6:
                belt_controller.send_vibration_command(
                    ...
                    orientation=DIRECTION_ANGLES[direction],
                    pattern_iterations=None,
                    pattern_period=100,
                    pattern_start_time=0,
                    exclusive_channel=False,
                    clear_other_channels=first
                )
                first = False
                channel += 1
    ...
    def stop_vibration(self, belt_controller):
        """Immediately silence the belt."""
        self.prev_intensities = {d: 0 for d in DIRECTIONS}
        self._prev_direction = None
        if belt_controller:
            belt_controller.stop_vibration()

    def pause(self, belt_controller):
        """Pause navigation (keeps route state)."""
        self.vibrate = False
        self.stop_vibration(belt_controller)
        print("[NavBelt] Navigation paused.")

    def resume(self):
        """Resume after pause."""
        self.vibrate = True
        self.mode = 'belt'
        print("[NavBelt] Navigation resumed.")

    def switch_to_belt_mode(self, belt_controller):
        """
        Switch back from bracelet to belt mode.
        Called if user moves away from target beyond switch distance.
        """
        self.mode = 'belt'
        self.was_guiding = False
        self.searching = True
        ...
        if belt_controller:
            belt_controller.stop_vibration()
        print("[NavBelt] Switched back to belt mode.")
    ...
    def reset(self):
        ...
        self.prev_intensities = {d: 0 for d in DIRECTIONS}
        self.last_vib_update_time = 0
        print("[NavBelt] State reset.")

    def get_status(self):
        """
        Return current navigation status for external queries (e.g. MCP).
        ...
        return {
            'mode': self.mode,
            'vibrating': self.vibrate,
            'was_guiding': self.was_guiding,
            'searching': self.searching,
            'switch_distance': self.switch_distance,
            'current_intensities': self.prev_intensities.copy(),
            'calibration': self.vibration_intensities.copy(),
            'navigation_active_since': self.navigation_time,
        }